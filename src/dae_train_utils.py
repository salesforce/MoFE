...
def get_relevant_deps_and_context(untokenize_line, nlp):
    tokenized_json = nlp.annotate(untokenize_line, properties={'annotators': 'tokenize', 'outputFormat': 'json',
                                                               'ssplit.isOneSentence': True})
    tokenized_text = []
    for tok in tokenized_json['tokens']:
        tokenized_text.append(tok['word'].lower())
    line = ' '.join(tokenized_text)
...
def get_single_features(decode_text, input_text, tokenizer, nlp):
